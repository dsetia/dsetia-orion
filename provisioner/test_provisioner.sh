...
OUTPUT=$(provisioner -config=./config/provision-dev-config.json -db=../config/testdb_ut.json -op provision-tenant -tenant-name "tenant1")
provisioner -config=./config/provision-dev-config.json -db=../config/testdb_ut.json -op provision-sensor -tenant-name "tenant1" --device-name "Device 1" -minio=../config/minio_config.json

echo "$OUTPUT"
TENANT_ID=$(echo "$OUTPUT" | grep -oE 'ID=[0-9]+' | cut -d= -f2)

# echo "Uploading images to minio"
objupdater -type software -dbconfig ../config/testdb_ut.json -minioconfig ../config/minio_config.json -file ../minio/hndr-sw-v1.2.3.tar.gz
objupdater -type rules -dbconfig ../config/testdb_ut.json -minioconfig ../config/minio_config.json -file ../minio/hndr-rules-r1.2.3.tar.gz -tenantid $TENANT_ID
objupdater -type threatintel -dbconfig ../config/testdb_ut.json -minioconfig ../config/minio_config.json -file ../minio/threatintel-2025.04.10.1523.tar.gz
...
./deploy.sh sensor ../config/minio_config.json $TENANT_ID

# pick up sensor package from minio generated by provisioning step above
mc cp myminio/sensor/$TENANT_ID/$TARBALL_PKG .

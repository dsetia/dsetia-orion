...
provisioner -config=$CFG_DIR/provisioner/provision-config.json -db=$CFG_DIR/db_dev_config.json -op provision-sensor -tenant-name "$TENANT_NAME" --device-name "$DEVICE_NAME" -minio=$CFG_DIR/minio_config.json
...
create-tarball.sh sensor $CFG_DIR $TENANT_ID
